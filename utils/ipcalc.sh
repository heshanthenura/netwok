...
show_menu() {
    ...
    echo "2) IP Range to CIDR"
    echo "3) Subnet a Network"
    echo "4) Supernetting (Merge Subnets)"
    echo "5) Check if IP Belongs to Subnet"
    echo "6) Calculate Usable Hosts"
    echo "7) Convert IP to Binary / Hex / Decimal"
    echo "8) Validate IP or Subnet Format"
